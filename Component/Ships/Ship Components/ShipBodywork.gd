...
func bodywork_drag(sideslip: float) -> void:
	...
	ship_rb.apply_force(-ship_rb.transform.basis.z * abs(sidedrag / 2))
...
func turning(turn_axis: float, forward_speed: float, ship_mesh: Node3D) -> void:
	...
	var speed_turn_deficit : float = 1 - forward_speed / 150
...
func turning_bank(turn_axis: float, speed_turn_deficit: float, ship_mesh: Node3D) -> void:
	ship_mesh.rotation.z = lerp(ship_mesh.rotation.z, -deg_to_rad(40 * turn_axis * speed_turn_deficit), 0.05)
	pass
